...
namespace QuanLyThuVien.Services
{
    public class MuonTraService
    {
            ...
            int soSachMuonMoi)
        {
            ...
            if (soSachMuonMoi < 1)
                return KetQuaXuLy.Loi(
                    "Phải chọn ít nhất 1 đầu sách.");

            if (soSachMuonMoi > 3)
                return KetQuaXuLy.Loi(
                    "Một lần lập phiếu chỉ được chọn tối đa 3 đầu sách khác nhau.");
            ...
            if (!daDongLePhi)
                return KetQuaXuLy.Loi(
                    "Độc giả chưa đóng lệ phí năm nên thẻ chưa có giá trị.");

            // Kiểm tra sách quá hạn chưa trả
            int quaHan = Convert.ToInt32(
                Db.Scalar(
                    @"SELECT COUNT(*)
                      FROM PhieuMuon pm
            ...
            // Số sách hiện đang mượn
            int dangMuon = Convert.ToInt32(
                Db.Scalar(
                    @"SELECT COUNT(*)
                      FROM PhieuMuon pm
                      JOIN ChiTietPhieuMuon ct
            ...
            if (dangMuon + soSachMuonMoi > 3)
                return KetQuaXuLy.Loi(
                    "Tổng số sách đang mượn và sắp mượn không được vượt quá 3 cuốn.");
            ...
            DateTime ngayHenTra)
        {
            if (maDauSach == null)
                return KetQuaXuLy.Loi(
                    "Danh sách sách mượn không hợp lệ.");

            // Loại bỏ mã trống và mã trùng
                                ...
                                tx))
                        {
                            ...
                            if (update.ExecuteNonQuery() != 1)
                                throw new InvalidOperationException(
                                    "Không thể cập nhật tồn kho cho "
                                    + maSach + ".");
                        }
        ...
        // =========================================================
        // 3. LẤY SÁCH ĐANG MƯỢN CỦA ĐỘC GIẢ
        // =========================================================
     ...
     tt.Equals(
         ...
         StringComparison.OrdinalIgnoreCase);
                    ...
                    // =============================================
                    // CHỈ TĂNG TỒN KHO NẾU KHÔNG MẤT/HƯ
                    // =============================================
                    if (!mat && !huHong)
                    {
